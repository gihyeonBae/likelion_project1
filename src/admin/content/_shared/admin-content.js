...
export function createBannerForm({ banner = {}, submitLabel }) {
  return `
    <form class="panel-form" id="banner-form">
      ...
      <label class="form-field">
        <span>버튼 링크</span>
        <input type="text" name="linkUrl" value="${banner.linkUrl ?? '/src/customer/menu/read/list'}" />
      </label>
      <label class="form-field">
        <span>노출 순서</span>
      ...
      <div class="detail-actions">
        ...
        <a class="button button--ghost" href="/src/admin/content/read/banner-list">목록</a>
      </div>
    </form>
  `;
}
  ...
  return `
    <article class="order-card">
      ...
      <div class="cart-item__actions">
        <a class="button button--ghost" href="/src/admin/content/update/banner?id=${banner.id}">수정</a>
        <a class="button button--ghost" href="/src/admin/content/delete?type=banner&id=${banner.id}">삭제</a>
      </div>
    </article>
  `;
}
