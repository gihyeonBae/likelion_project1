...
if (order && !requestedOrderId) {
  window.history.replaceState(null, '', `/src/customer/payment/create?id=${order.id}`);
}
...
if (!order) {
    ...
    <article class="cart-form-layout">
      ...
      <form class="panel-form" id="payment-form">
        ...
        <div class="detail-actions">
          ...
          <a class="button button--ghost" href="/src/customer/order/read/detail?id=${order.id}">주문 상세</a>
        </div>
      </form>
    </article>
  `;
  ...
  document.getElementById('payment-form').addEventListener('submit', async (event) => {
    ...
    try {
      ...
      window.location.href = `/src/customer/order/read/complete?id=${order.id}`;
    } catch (paymentError) {
